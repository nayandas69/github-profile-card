/**
 * SVG icon paths used in the profile card.
 * Each icon is an SVG path string from GitHub's Octicons set.
 *
...
/**
 * Renders an inline SVG icon string for embedding in the card.
 *
 * @param name  - Icon identifier from the icons map
...
export function icon(name: IconName, color: string, size = 16): string {
  ...
  return `<svg width="${size}" height="${size}" viewBox="${viewBox}" fill="#${color}" role="img">${path}</svg>`;
}
